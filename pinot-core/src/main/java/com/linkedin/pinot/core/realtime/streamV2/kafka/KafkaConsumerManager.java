...
public class KafkaConsumerManager {
  ...
  public static KafkaConsumerAndIterator acquireConsumerAndIteratorForConfig(KafkaStreamConfig kafkaStreamConfig) {

    final ImmutableTriple<String, String, String> configKey =
        new ImmutableTriple<>(kafkaStreamConfig.getKafkaTopicName(), kafkaStreamConfig.getGroupId(),
            kafkaStreamConfig.getZkBrokerUrl());
    ...
    synchronized (KafkaConsumerManager.class) {
      ...
      LOGGER.info("Creating new kafka consumer and iterator for topic {}", kafkaStreamConfig.getKafkaTopicName());

      // Create the consumer
      ConsumerConnector consumer =
          kafka.consumer.Consumer.createJavaConsumerConnector(kafkaStreamConfig.getKafkaConsumerConfig());

      // Create the iterator (can only be done once per consumer)
      ConsumerIterator<byte[], byte[]> iterator = consumer.createMessageStreams(kafkaStreamConfig.getTopicMap(1)).
          get(kafkaStreamConfig.getKafkaTopicName()).get(0).iterator();
      ...
      LOGGER.info("Created consumer/iterator with id {} for topic {}", consumerAndIterator.getId(),
          kafkaStreamConfig.getKafkaTopicName());
        ...
        try {
          ...
          synchronized (KafkaConsumerManager.class) {
            ...
            while (configIterator.hasNext()) {
              Map.Entry<ImmutableTriple<String, String, String>, KafkaConsumerAndIterator> entry =
                  configIterator.next();
              KafkaConsumerAndIterator consumerAndIterator1 = entry.getValue();

              final Long releaseTime1 = CONSUMER_RELEASE_TIME.get(consumerAndIterator1);
              if (!releaseTime1.equals(IN_USE) && releaseTime1 < System.currentTimeMillis()) {
                LOGGER.info("Releasing consumer/iterator {}", consumerAndIterator1.getId());

                try {
                  consumerAndIterator1.getConsumer().shutdown();
                } catch (Exception e) {
                  LOGGER.warn("Caught exception while shutting down Kafka consumer with id {}",
                      consumerAndIterator1.getId(), e);
                }

                configIterator.remove();
                CONSUMER_RELEASE_TIME.remove(consumerAndIterator1);
              } else {
                LOGGER.info("Not releasing consumer/iterator {}, it has been reacquired", consumerAndIterator1.getId());
              }
            }
          }
        } catch (Exception e) {
