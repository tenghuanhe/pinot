...
public class HLRealtimeSegmentDataManagerV2 extends RealtimeSegmentDataManager {
  private static final Logger LOGGER = LoggerFactory.getLogger(HLRealtimeSegmentDataManagerV2.class);
  ...
  private HighLevelStreamConsumer _highLevelStreamConsumer;
  ...
  public HLRealtimeSegmentDataManagerV2(final RealtimeSegmentZKMetadata realtimeSegmentZKMetadata,
    ...
    indexingThread = new Thread(() -> {
      ...
      do {
        ...
        try {
          ...
          byte[] message = _highLevelStreamConsumer.fetchNextMessage();
          _streamMessageDecoder.decode(message, readRow);
          row = readRow;
      ...
      try {
        ...
        if ((numErrors = extractor.getTotalErrors()) > 0) {
          serverMetrics.addMeteredTableValue(_tableStreamName, ServerMeter.ROWS_WITH_ERRORS, (long) numErrors);
        }
        ...
        for (String column : errorCount.keySet()) {
          if ((numErrors = errorCount.get(column)) > 0) {
            _segmentLogger.warn("Column {} had {} rows with errors", column, numErrors);
          }
        }
        if ((numConversions = extractor.getTotalConversions()) > 0) {
          serverMetrics.addMeteredTableValue(_tableStreamName, ServerMeter.ROWS_NEEDING_CONVERSIONS,
              (long) numConversions);
          _segmentLogger.info("{} rows needed conversions ", numConversions);
        }
        if ((numNulls = extractor.getTotalNulls()) > 0) {
          serverMetrics.addMeteredTableValue(_tableStreamName, ServerMeter.ROWS_WITH_NULL_VALUES, (long) numNulls);
          _segmentLogger.info("{} rows had null columns", numNulls);
        }
        if ((numNullCols = extractor.getTotalNullCols()) > 0) {
          serverMetrics.addMeteredTableValue(_tableStreamName, ServerMeter.COLUMNS_WITH_NULL_VALUES, (long) numNullCols);
          _segmentLogger.info("{} columns had null values", numNullCols);
        }
        _segmentLogger.info("Indexing threshold reached, proceeding with index conversion");
        // kill the timer first
        segmentStatusTask.cancel();
        updateCurrentDocumentCountMetrics();
        _segmentLogger.info("Indexed {} raw events", realtimeSegment.getNumDocsIndexed());
        ...
        RealtimeSegmentConverter converter =
            new RealtimeSegmentConverter(realtimeSegment, tempSegmentFolder.getAbsolutePath(), schema,
                ...
                sortedColumn, HLRealtimeSegmentDataManagerV2.this.invertedIndexColumns, noDictionaryColumns,
        ...
        _segmentLogger.info("Committing Kafka offsets");
        boolean commitSuccessful = false;
        try {
          _highLevelStreamConsumer.commit();
          commitSuccessful = true;
          _highLevelStreamConsumer.close();
          _segmentLogger.info("Successfully committed Kafka offsets, consumer release requested.");
        } catch (Throwable e) {
  ...
  private void computeKeepIndexing() {
    if (keepIndexing) {
      _segmentLogger.debug("Current indexed {} raw events", realtimeSegment.getNumDocsIndexed());
  ...
  private void updateCurrentDocumentCountMetrics() {
    ...
    serverMetrics.addValueToTableGauge(tableName, ServerGauge.DOCUMENT_COUNT,
