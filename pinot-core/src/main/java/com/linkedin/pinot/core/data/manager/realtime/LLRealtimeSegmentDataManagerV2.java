...
import com.linkedin.pinot.core.realtime.streamV2.StreamConsumerFactoryProvider;
import com.linkedin.pinot.core.realtime.streamV2.StreamMessageDecoder;
import com.linkedin.pinot.core.realtime.streamV2.StreamMessageDecoderFactory;
 ...
 */
public class LLRealtimeSegmentDataManagerV2 extends RealtimeSegmentDataManager {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(LLRealtimeSegmentDataManagerV2.class);
  ...
  private StreamMessageDecoder _streamMessageDecoder;
  private SimpleStreamConsumer _simpleStreamConsumer;
  private StreamMetadataProvider _streamMetadataProvider;
  ...
  protected boolean consumeLoop() throws Exception {
    ...
    final long maxIdleCountBeforeStatUpdate =
        (3 * 60 * 1000) / (idlePipeSleepTimeMillis + _streamConfig.getFetchTimeoutMillis());  // 3 minute count
    ...
    while (!_shouldStop && !endCriteriaReached()) {
      // Consume for the next fetch timeout ms, or we get to final offset, whichever happens earlier,
      // Update _currentOffset upon return from this method
      MessageBatch messageBatch;
      try {
        messageBatch = _simpleStreamConsumer.fetchMessages(_currentOffset, _endOffset,
            (int) _streamConfig.getFetchTimeoutMillis());
        ...
        segmentLogger.warn("Permanent exception when fetching messages, stopping consumption", e);
        throw e;
      } catch (Exception e) {
      ...
      if (_currentOffset != lastUpdatedOffset) {
        ...
        // FIXME: kafka metric incremented here
  ...
  private void processStreamEvents(MessageBatch messagesAndOffsets, long idlePipeSleepTimeMillis) {
    ...
    for (int index = 0; index < messagesAndOffsets.getMessageCount(); index++) {
      ...
      if (!canTakeMore) {
        ...
        //    Throw an exception.
        //
        // 2. We are in CATCHING_UP state, and we legally hit this error due to simple stream unclean leader election where
      ...
      decodedRow = _streamMessageDecoder.decode(messagesAndOffsets.getMessageAtIndex(index),
    ...
    if (streamMessageCount != 0) {
      ...
      // If there were no messages to be fetched from stream, wait for a little bit as to avoid hammering the broker
  ...
  protected SegmentBuildDescriptor buildSegmentInternal(boolean forCommit) {
    try {
      ...
      if (forCommit) {
        return new SegmentBuildDescriptor(destDir.getAbsolutePath() + TarGzCompressionUtils.TAR_GZ_FILE_EXTENTION,
  ...
  private SegmentCompletionProtocol.Response doSplitCommit(SegmentCompletionProtocol.Response prevResponse) {
    ...
    params.withSegmentName(_segmentNameStr)
  ...
  protected SegmentCompletionProtocol.Response postSegmentConsumedMsg() {
    ...
    SegmentCompletionProtocol.Request.Params params = new SegmentCompletionProtocol.Request.Params();
    params.withOffset(_currentOffset)
        .withSegmentName(_segmentNameStr)
  ...
  public LLRealtimeSegmentDataManagerV2(RealtimeSegmentZKMetadata segmentZKMetadata, TableConfig tableConfig,
      InstanceZKMetadata instanceZKMetadata, RealtimeTableDataManager realtimeTableDataManager, String resourceDataDir,
      IndexLoadingConfig indexLoadingConfig, Schema schema, ServerMetrics serverMetrics) throws Exception {
    ...
    if (segmentPartitionConfig != null) {
      try {
        ...
        makeMetadataFetcher("Timeout getting number of partitions");
      }
    }
    ...
    _consumeEndTime = now + _streamConfig.getFlushThresholdTimeMillis();
