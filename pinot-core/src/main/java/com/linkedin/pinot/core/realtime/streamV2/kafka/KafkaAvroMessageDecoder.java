...
public class KafkaAvroMessageDecoder implements StreamMessageDecoder<byte[]> {
  ...
  private static final int SCHEMA_FETCHER_TIMEOUT_MILLIS = 15000;
  ...
  public void init(Map<String, String> props, Schema indexingSchema, String topicName) throws Exception {
    ...
    this._avroRecordConverter = new AvroRecordToPinotRowGenerator(indexingSchema);
  ...
  private static class SchemaFetcher implements Callable<Boolean> {
    ...
    public Boolean call() {
      try {
        ...
        conn.setConnectTimeout(SCHEMA_FETCHER_TIMEOUT_MILLIS);
        conn.setReadTimeout(SCHEMA_FETCHER_TIMEOUT_MILLIS);
